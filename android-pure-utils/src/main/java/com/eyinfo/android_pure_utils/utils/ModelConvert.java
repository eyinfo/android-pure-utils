...
public class ModelConvert {
    ...
    public static <T, RT> List<T> toModels(List<RT> source, Class<T> targetClass, boolean deep, Action1<T> itemAction) {
        if (source == null) {
            return null;
        }
        List<T> items = new ArrayList<>();
        for (RT rt : source) {
            T model = toModel(rt, targetClass, deep);
            if (itemAction == null) {
                specialFieldResolution(rt, model);
            } else {
    ...
    public static <T, RT> List<T> toModels(List<RT> source, Class<T> targetClass, Action1<T> itemAction) {
    ...
    public static <T> List<T> toModels(Object source, Class<T> targetClass) {
        String content = JsonUtils.toJson(source);
